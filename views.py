...
def submit(request, course_id):
    ...
    # Collect the selected choices from the request.POST
    selected_choice_ids = []
...
def show_exam_result(request, course_id, submission_id):
    ...
    for question in course.question_set.all():
        is_correct = set(question.correct_choices.values_list('id', flat=True)) == set(
            selected_choice_ids)  # Check if all selected choices are correct
        score = question.score if is_correct else 0  # Assign score based on correctness
        total_score += score
